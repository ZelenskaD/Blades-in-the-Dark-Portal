...
class Stress:
    ...
    def __repr__(self):
        return f"<Stress(current={self.current}, maximum={self.maximum})>"


class StressType(TypeDecorator):
    impl = JSON
    ...
    def process_bind_param(self, value, dialect):
        ...
        """
        if value is not None and isinstance(value, Stress):
    ...
    def process_result_value(self, value, dialect):
        ...
        if value is not None:
            return Stress.from_dict(value)
        return value


# Use MutableList to track changes in the JSON column
MutableDict.associate_with(StressType)
